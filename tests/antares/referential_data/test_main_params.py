...
def test_main_params_mandatory_sheets(tmp_path: Path) -> None:
    # given
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("Missing sheets: ['PAYS', 'STUDY_SCENARIO', 'CLUSTER', 'PEAK_PARAMS']"),
    ):
        parse_main_params(file_path=path_file)


def test_main_params_mandatory_columns(tmp_path: Path) -> None:
    # given
    path_file = tmp_path / "MAIN_PARAMS.xlsx"
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("Columns mismatch in sheet 'PAYS'. Expected:"),
    ):
        parse_main_params(file_path=path_file)


def test_main_params_works(tmp_path: Path) -> None:
    # given
    path_file = tmp_path / "MAIN_PARAMS.xlsx"
    ...
    assert isinstance(main_params.pays, pd.DataFrame)
    assert isinstance(main_params.study_scenario, pd.DataFrame)
    assert isinstance(main_params.cluster, pd.DataFrame)
    assert isinstance(main_params.peak_params, pd.DataFrame)

    pd.testing.assert_frame_equal(main_params.pays, ref_pays, check_dtype=False)
    pd.testing.assert_frame_equal(main_params.study_scenario, ref_scenario, check_dtype=False)
    pd.testing.assert_frame_equal(main_params.cluster, ref_cluster, check_dtype=False)
    pd.testing.assert_frame_equal(main_params.peak_params, ref_peak_params, check_dtype=False)
