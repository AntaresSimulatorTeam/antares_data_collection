...
def test_main_params_file_not_exist(tmp_path: Path):
    # given
...
def test_main_params_default_sheet_names(tmp_path: Path):
    # given
    ...
    with pytest.raises(
            ValueError,
            match=re.escape(f"Sheet 'test' not found in MAIN_PARAMS.xlsx"),
    ):
        MainParams(path_file=path_file)


def test_main_params_with_bad_name_columns(tmp_path: Path):
    # given
    path_file = tmp_path / "MAIN_PARAMS.xlsx"
    ...
    with pytest.raises(
            ValueError,
            match=re.escape(f"Columns names mismatch for sheet 'PAYS'"),
    ):
        MainParams(path_file=path_file)


def test_main_params_works_one_ref(tmp_path: Path):
    # given
    path_file = tmp_path / "MAIN_PARAMS.xlsx"
